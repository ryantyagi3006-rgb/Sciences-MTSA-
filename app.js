  ...
  return `
    <div class="worksheet-page fade-in">
      <div class="worksheet-toolbar">
        ...
        <button id="notes-btn" class="btn btn-secondary${_notepadOpen ? ' notes-active' : ''}" onclick="toggleNotepad()">Notes</button>
        <button class="btn btn-secondary${isBookmarked(sheetId) ? ' notes-active' : ''}"
          onclick="openBookmarkModal('${sheet.id}')">
          ${isBookmarked(sheetId) ? '&#9733; Bookmarked' : '&#9734; Bookmark'}
        </button>
        ...
        <div class="file-card stagger-${Math.min(i+1,5)} fade-in-up"
          ...
          <div class="file-info">
            <div class="file-title-row">
              ...
              <button class="card-bm-btn${bmd ? ' bm-active' : ''}"
                onclick="event.stopPropagation();openBookmarkModal('${s.id}')"
                title="${bmd ? 'Bookmarked — click to edit' : 'Bookmark this sheet'}">${bmd ? '&#9733;' : '&#9734;'}</button>
            </div>
      ...
      ${searched.map((s, i) => {
        ...
        <div class="file-list-row stagger-${Math.min(i+1,5)} fade-in"
          ...
          <div class="file-list-badges">
            <span class="crit-badge ${criterionClass(s.criterion)}">Crit. ${critLabel(s.criterion)}</span>
            <button class="bm-list-btn${bmd ? ' bm-active' : ''}"
              onclick="event.stopPropagation();openBookmarkModal('${s.id}')"
  ...
  return `
    <div class="sidebar ${state.sidebarOpen ? 'open' : ''}" id="sidebar">
      <div class="sidebar-logo">
        <div class="logo-icon">
          <span class="logo-icon-text">MYP</span>
        </div>
        <div>
      ...
      <nav class="sidebar-nav">
        ...
        <button class="nav-item ${state.view === 'bookmarks' ? 'active' : ''}"
          onclick="navigateBookmarks()">
          <span style="font-size:13px;margin-right:2px">&#9733;</span>
          Bookmarks
          ${getBookmarks().length > 0 ? `<span style="margin-left:auto;font-size:11px;font-weight:700;color:var(--text-muted)">${getBookmarks().length}</span>` : ''}
        </button>
...
function renderApp() {
  ...
  if (state.view === 'dashboard') {
    ...
    pageContent = renderBookmarksView();
  }
  ...
  // Restore saved answers and notepad when opening a sheet
  if (state.view === 'sheet') {
    loadAnswers(state.sheetId);
    if (_notepadOpen) loadNotepadContent();
    const panel = document.getElementById('notepad-panel');
    if (panel) panel.classList.toggle('open', _notepadOpen);
  } else {
    // Hide notepad when leaving sheet view
    _notepadOpen = false;
    const panel = document.getElementById('notepad-panel');
...
function downloadAnswers(sheetId) {
  ...
  const critColor = { A: '#D93040', BC: '#B05A00', D: '#0369A1' };
  const subColor  = { biology: '#0B9467', chemistry: '#1A5FCF', physics: '#C96D00' };
...
function getBookmarks() {
  try { return JSON.parse(localStorage.getItem('myp-bookmarks') || '[]'); } catch { return []; }
}
function saveBookmarks(bms) {
  try { localStorage.setItem('myp-bookmarks', JSON.stringify(bms)); } catch {}
}
function isBookmarked(sheetId) {
  return getBookmarks().some(b => b.sheetId === sheetId);
}

let _bmTargetId = null;

function openBookmarkModal(sheetId) {
  _bmTargetId = sheetId;
  const sheet = getSheet(sheetId);
  const sub = subjectMeta(sheet.subject);
  const existing = getBookmarks().find(b => b.sheetId === sheetId);
  document.getElementById('bm-modal-sheet-title').textContent = sheet.title;
  document.getElementById('bm-modal-sheet-meta').textContent = sub.name + ' · Criterion ' + sheet.criterion;
  document.getElementById('bm-email').value = existing ? existing.email : '';
  document.getElementById('bm-due-date').value = existing ? existing.dueDate : '';
  document.getElementById('bm-modal-overlay').classList.add('open');
}

function closeBookmarkModal() {
  const el = document.getElementById('bm-modal-overlay');
  if (el) el.classList.remove('open');
  _bmTargetId = null;
}

function confirmBookmark() {
  if (!_bmTargetId) return;
  const sheet = getSheet(_bmTargetId);
  const sub = subjectMeta(sheet.subject);
  const email = document.getElementById('bm-email').value.trim();
  const dueDate = document.getElementById('bm-due-date').value;
  const bms = getBookmarks().filter(b => b.sheetId !== _bmTargetId);
  const bm = {
    sheetId: _bmTargetId, sheetTitle: sheet.title,
    subject: sheet.subject, subjectName: sub.name,
    criterion: sheet.criterion, criterionName: sheet.criterionName,
    bookmarkedAt: new Date().toISOString(), dueDate, email, dueDateEmailSent: false,
  };
  bms.push(bm);
  saveBookmarks(bms);

  if (email && dueDate) {
    const dueFmt = new Date(dueDate + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
    const subj = encodeURIComponent('Reminder set: ' + sheet.title + ' due ' + dueFmt);
    const body = encodeURIComponent(
      'Hi,\n\nYou\'ve bookmarked the following MYP Sciences task sheet:\n\n' +
      '  "' + sheet.title + '"\n  Subject: ' + sub.name + '\n  Criterion: ' + sheet.criterionName +
      '\n  Due date: ' + dueFmt + '\n\nYou\'ll receive another reminder on the due date.\n\nGood luck!\nMYP Sciences Drive'
    );
...
function removeBookmark(sheetId) {
  saveBookmarks(getBookmarks().filter(b => b.sheetId !== sheetId));
  renderApp();
}
...
function checkDueDateReminders() {
  ...
  let updated = false;
  bms.forEach(bm => {
    if (bm.dueDate === today && !bm.dueDateEmailSent && bm.email) {
      const dueFmt = new Date(bm.dueDate + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
      const subj = encodeURIComponent('Due today: ' + bm.sheetTitle);
      const body = encodeURIComponent(
        'Hi,\n\nThis is your reminder that the following task sheet is due TODAY:\n\n' +
        '  "' + bm.sheetTitle + '"\n  Subject: ' + bm.subjectName + '\n  Criterion: ' + bm.criterionName +
        '\n  Due: ' + dueFmt + '\n\nGood luck!\nMYP Sciences Drive'
      );
      window.open('mailto:' + bm.email + '?subject=' + subj + '&body=' + body);
      bm.dueDateEmailSent = true;
      updated = true;
    }
  });
...
// ─── NOTEPAD ──────────────────────────────────────────────────────────────
let _notepadOpen = false;

function toggleNotepad() {
  _notepadOpen = !_notepadOpen;
  const panel = document.getElementById('notepad-panel');
  if (panel) panel.classList.toggle('open', _notepadOpen);
  const btn = document.getElementById('notes-btn');
  if (btn) btn.classList.toggle('notes-active', _notepadOpen);
  if (_notepadOpen) loadNotepadContent();
}
...
function saveNotepadContent() {
  ...
  if (!ta) return;
  try { localStorage.setItem('myp-notes-' + state.sheetId, ta.value); } catch {}
  const cc = document.getElementById('notepad-char-count');
...
function loadNotepadContent() {
  ...
  if (!ta) return;
  try {
    const saved = localStorage.getItem('myp-notes-' + state.sheetId) || '';
    ta.value = saved;
    const cc = document.getElementById('notepad-char-count');
    if (cc) cc.textContent = saved.length + ' characters';
  } catch {}
}
...
function mountOverlays() {
  if (!document.getElementById('bm-modal-overlay')) {
    const el = document.createElement('div');
    el.id = 'bm-modal-overlay';
    el.className = 'modal-overlay';
    el.innerHTML = `
      <div class="modal" onclick="event.stopPropagation()">
        <div class="modal-header">
          <h3>Bookmark Task Sheet</h3>
          <button class="modal-close" onclick="closeBookmarkModal()">&#x2715;</button>
        </div>
        <div class="modal-body">
          <div class="modal-sheet-info">
            <strong id="bm-modal-sheet-title"></strong>
            <span id="bm-modal-sheet-meta"></span>
          </div>
          <div class="form-field">
            <label>Email address for reminders</label>
            <input type="email" id="bm-email" placeholder="you@example.com" />
            <div class="form-hint">We'll open your email app with a confirmation message now, and another on the due date.</div>
          </div>
          <div class="form-field">
            <label>Due date</label>
            <input type="date" id="bm-due-date" />
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" onclick="closeBookmarkModal()">Cancel</button>
          <button class="btn btn-primary" onclick="confirmBookmark()">Save Bookmark</button>
        </div>
      </div>
    `;
    el.addEventListener('click', closeBookmarkModal);
    document.body.appendChild(el);
  }

  if (!document.getElementById('notepad-panel')) {
    const el = document.createElement('div');
    el.id = 'notepad-panel';
    el.className = 'notepad-panel';
    el.innerHTML = `
      <div class="notepad-header">
        <div class="notepad-header-left">
          <div class="notepad-title">Notes</div>
          <div class="notepad-sub">Saved automatically per sheet</div>
        </div>
        <button class="notepad-close" onclick="toggleNotepad()">&#x2715;</button>
      </div>
      <textarea id="notepad-textarea" class="notepad-textarea"
        placeholder="Jot down thoughts, key formulas, or rough working…"
        oninput="saveNotepadContent()"></textarea>
      <div class="notepad-footer">
        <span id="notepad-char-count" class="notepad-char-count">0 characters</span>
...
function renderBookmarksView() {
  ...
  if (bms.length === 0) {
    rowsHtml = `<div class="empty-state">
      ...
      <p>Click the star on any task sheet card to bookmark it and set a reminder.</p>
    </div>`;
  } else {
    bms.slice().reverse().forEach(bm => {
      const sub = subjectMeta(bm.subject);
      let dueBadge = '';
      if (bm.dueDate) {
        const diff = Math.round((new Date(bm.dueDate + 'T00:00:00') - new Date(today + 'T00:00:00')) / 86400000);
        let cls = '', label = '';
        if (diff < 0)       { cls = '';          label = 'Overdue by ' + Math.abs(diff) + 'd'; }
        else if (diff === 0){ cls = 'due-today'; label = 'Due today'; }
        else if (diff <= 3) { cls = 'due-soon';  label = 'Due in ' + diff + 'd'; }
        else                { label = 'Due ' + new Date(bm.dueDate + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }); }
        dueBadge = `<span class="bm-due-badge ${cls}">${label}</span>`;
      }
      rowsHtml += `
        <div class="bm-row">
          <div class="bm-icon subj-abbr ${bm.subject}">${sub.abbr}</div>
          <div class="bm-info">
            <div class="bm-title">${bm.sheetTitle}</div>
            <div class="bm-meta">${bm.subjectName} &middot; Criterion ${critLabel(bm.criterion)}</div>
          </div>
          <div class="bm-actions">
            ${dueBadge}
            <button class="btn btn-secondary" style="font-size:12px;padding:5px 12px" onclick="navigateSheet('${bm.sheetId}')">Open</button>
            <button class="bm-remove-btn" onclick="removeBookmark('${bm.sheetId}')" title="Remove bookmark">&#x2715;</button>
          </div>
        </div>
      `;
    });
  ...
  return `<div class="fade-in">
    <div class="page-header">
      <div class="breadcrumb"><span class="bc-item current">Bookmarks</span></div>
      <div class="page-title">Bookmarks</div>
      <div class="page-subtitle">${bms.length} saved sheet${bms.length !== 1 ? 's' : ''}</div>
    </div>
    <div class="bookmarks-grid">${rowsHtml}</div>
  </div>`;
}
